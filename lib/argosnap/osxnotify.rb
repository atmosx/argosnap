  ...
  class OSXNotifications
    ...
    def install_launchd_script 
      ...
      begin
        ...
        start_script = File.expand_path("../../files/local.sh")
        if Dir.exists?(launch_agents)
          filename = "org.#{user}.argosnap.plist"
          hash = {"Label"=>"#{filename.a.scan(/(.+)(?:\.[^.]+)/).flatten[0]}", "ProgramArguments"=>["#{start_script}"], "StartInterval"=> @time_interval, "RunAtLoad"=>true} 
